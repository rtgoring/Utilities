#Todo, read each file, count how many of each item
import shutil
import os
...
coppiedAnnotations = 0	# Number of Annotatiosn coppied to working directory
coppiedImages = 0	# Number of Images Coppied to working directory
annotatedItems = 0	# Number of Annotated Objects, includes multiple per image
...
for f in parentFolders:
	...
	try:
		...
		print "failed coppying annotation"
	...
	try:
		...
		print "failed coppying JPEGImages"
...
os.rename(os.path.join(datasetName,CFGModel + '-'+ str(len(classes)) + '.cfg'),os.path.join(datasetName,datasetName+'.cfg'))
...
#print "Datafile created: " +'cfg/'+datasetName+'.data\n' 
...
try:
    ...
    print "can not open MetaDataWriter"
...
metaDataWriter.write("../darknet detector train " +datasetName+'.data ' +datasetName+'.cfg' +  " ../darknet19_448.conv.23\n\n") 
metaDataWriter.write("Demo Script\n")
metaDataWriter.write("../darknet detector demo " + datasetName + ".data " + datasetName + ".cfg " +  "Models/" + datasetName+"_900.weights ../aaa.mp4\n") 
